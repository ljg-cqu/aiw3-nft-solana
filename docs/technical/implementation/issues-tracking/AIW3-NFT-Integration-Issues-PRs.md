...
| NFT-007 | Create NFTService: Qualification Logic | `_feat` | 🔲 Todo | **Affects**: Creates new api/services/NFTService.js<br>**Changes**: Adds qualification checking system with badge activation requirements using existing RedisService | **Needs**: Access to Trades model, User model, Badge model, existing RedisService (ioredis client), trading volume aggregation from Trades table | **Solution**: Create service to aggregate trading volume from Trades.total_usd_price, check activated badge count from Badge.status='activated', implement badge requirement logic (Level 2: 2 badges, Level 3: 4 badges, Level 4: 5 badges, Level 5: 6 badges), cache results using `RedisService.setCache('nft_qual:{userId}', {tradingVolume, activatedBadges, eligibleLevels}, 300)`, implement distributed locking with `RedisService.setCache(lockKey, 'locked', ttl, {lockMode: true})` | 🟡 Medium Risk<br>🔥 Critical | NFT-004 |
