...
|----------|-------|------|----------|---------------|-----------------------------------|-------------------|-----------------|-------------|
| NFT-001 | Database Migration: Create NFT Core Tables | `_config` | 🔲 Todo | **Affects**: MySQL database schema<br>**Changes**: Adds 4 new tables (UserNft, UserNftQualification, Badge, NftDefinition) with badge status tracking | **Needs**: Database admin access, migration framework, existing User table structure | **Solution**: Create Waterline models with proper relationships, badge status enum ('owned', 'activated', 'consumed'), indexes on user_id, badge_identifier, and status fields, foreign key constraints to User table | 🔴 High Risk<br>🔥 Critical | None |
...
| NFT-007 | Create NFTService: Qualification Logic | `_feat` | 🔲 Todo | **Affects**: Creates new api/services/NFTService.js<br>**Changes**: Adds qualification checking system with badge activation requirements using existing RedisService | **Needs**: Access to Trades model, User model, Badge model, existing RedisService (ioredis client), trading volume aggregation from Trades table | **Solution**: Create service to aggregate trading volume from Trades.total_usd_price (includes perpetual contract and strategy trading volume from complete trading history - pre-NFT and post-NFT launch), check activated badge count from Badge.status='activated', implement badge requirement logic (Level 2: 2 badges, Level 3: 4 badges, Level 4: 5 badges, Level 5: 6 badges), cache results using `RedisService.setCache('nft_qual:{userId}', {tradingVolume, activatedBadges, eligibleLevels}, 300)`, implement distributed locking with `RedisService.setCache(lockKey, 'locked', ttl, {lockMode: true})` | 🟡 Medium Risk<br>🔥 Critical | NFT-004 |
| NFT-008 | Create NFTService: Upgrade Processing | `_feat` | 🔲 Todo | **Affects**: NFTService.js, transaction handling<br>**Changes**: Implements burn-and-mint workflow with badge consumption | **Needs**: Web3Service methods, database transaction support, user authentication, error recovery, Badge model updates | **Solution**: Implement atomic upgrade process: verify qualification (volume + activated badges) → burn old NFT → mint new NFT → update database → consume activated badges (status='activated' to 'consumed', consumed_at=NOW()) → publish badge consumption events, with rollback on failure | 🔴 High Risk<br>🔥 Critical | NFT-005, NFT-007 |
...
| NFT-015 | Create NFTController: Badge Management | `_feat` | 🔲 Todo | **Affects**: NFTController.js<br>**Changes**: Adds GET /api/nft/badges endpoints for badge collection display<br>**Frontend Integration**: Badge collection UI, activation buttons, status indicators | **Needs**: Badge model access, user badge data, existing request/response patterns | **Solution**: Create endpoints for badge listing with status (owned/activated/consumed), badge details, earned date tracking. Response format: `{badges: [{badgeId, badgeName, status, earnedDate, imageUrl, description}], totalBadges, ownedBadges}` | 🟡 Medium Risk<br>⭐ High | NFT-010 |
| NFT-015a | Create NFTController: Badge Activation | `_feat` | 🔲 Todo | **Affects**: NFTController.js<br>**Changes**: Adds POST /api/nft/badges/:badgeId/activate endpoint<br>**Frontend Integration**: Badge activation flow, qualification status updates | **Needs**: Badge model status updates, NFT qualification cache invalidation, real-time status broadcasting | **Solution**: Implement badge activation endpoint that transitions badges from 'owned' to 'activated' status, clears qualification cache using `RedisService.delCache('nft_qual:{userId}')`, publishes badge activation events via Kafka for real-time UI updates | 🟡 Medium Risk<br>🔥 Critical | NFT-015 |
| NFT-016 | Update Routes Configuration | `_config` | 🔲 Todo | **Affects**: config/routes.js<br>**Changes**: Adds NFT and badge API routes with middleware | **Needs**: Existing route patterns, authentication middleware, rate limiting configuration | **Solution**: Add NFT routes to routes.js, apply authentication middleware, configure rate limiting, ensure consistent URL patterns. Include badge activation route: POST /api/nft/badges/:badgeId/activate | 🟡 Medium Risk<br>⭐ High | NFT-011, NFT-012, NFT-013, NFT-014, NFT-015, NFT-015a |
...
| NFT-021 | Kafka Integration: NFT Events | `_feat` | 🔲 Todo | **Affects**: api/services/KafkaService.js, event publishing<br>**Changes**: Adds NFT and badge events using existing KafkaService infrastructure | **Needs**: Existing KafkaService (kafkajs client, broker: host.docker.internal:29092), "nft-events" topic creation, message schema standards | **Solution**: Use `KafkaService.sendMessage('nft-events', {eventType, timestamp, data})` for NFT and badge events, implement structured message format: `{eventType: 'claimed/upgraded/qualified/badge_activated/badges_consumed', timestamp: ISO8601, data: {userId, nftLevel, tierName, badgeId, badgeName, etc}}`, ensure frontend WebSocket integration for real-time badge status updates | 🟡 Medium Risk<br>📋 Medium | NFT-017 |
...
| NFT-038 | WebSocket Events: NFT Updates | `_feat` | 🔲 Todo | **Affects**: Real-time communication, existing socket infrastructure<br>**Changes**: Adds NFT and badge real-time updates<br>**Frontend Integration**: Real-time Personal Center updates, live transaction status, badge activation notifications, qualification notifications | **Needs**: Existing WebSocket setup, socket.io configuration, event broadcasting patterns, client-side event handling | **Solution**: Extend socket infrastructure with NFT and badge-specific events: `nft:status_changed`, `nft:upgrade_progress`, `nft:qualification_updated`, `nft:transaction_status`, `badge:activated`, `badge:status_changed`. Include user-specific room management for targeted updates. Provide JavaScript SDK for easy frontend integration with event handlers and automatic UI updates for badge collection | 🟡 Medium Risk<br>⭐ High | NFT-034 |
